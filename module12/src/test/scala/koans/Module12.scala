...
class Module12 extends FunSpec with Matchers with StopOnFirstFailure with SeveredStackTraces {
  ...
  class ObscureMathUtil {
    ...
    //We need to understand a lot of things here
    //1. Notice the looping structure, nice trick to loop for a certain number of times
    //2. next is assigned the current value and next + acc is the accumulator
  }
  ...
  object RockPaperScissors extends TailRec[Outcome.Value] { //This is how we use Enumeration as Type
